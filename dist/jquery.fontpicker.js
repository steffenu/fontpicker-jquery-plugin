	...
	$.fn.fontpicker = function (options) {
		...
		var __googleFonts = {
			...
			"Bai Jamjuree": {
				category: "sans-serif",
				variants:
					"200,200i,300,300i,400,400i,500,500i,600,600i,700,700i",
				subsets: "latin,latin-ext,thai,vietnamese",
			},
			"Baloo 2": {
				category: "display",
			...
			"Merriweather Sans": {
				category: "sans-serif",
				variants:
					"300,400,500,600,700,800,300i,400i,500i,600i,700i,800i",
		 ...
		 var __adobeFonts = {
			...
			Fontpicker.prototype = {
						...
						jQuery('<button type="button" class="fp-btn apply">')
							...
							.on("click", function (e) {
								...
								if (!!self.options.nrRecents) {
									...
									recentFonts = recentFonts.slice(
										0,
										self.options.nrRecents
									); // Remember last X
				...
				applyFontToOriginalInput: function (fontSpec) {
					...
					this.loadFont(
						this.options.googleFonts !== false &&
							__googleFonts[font.family]
							? "google"
							: "local",
						font.family
					);
				...
				getFavorites: function () {
					...
					favoriteFonts = favoriteFonts
						...
						: [];
					recentFonts =
						!!this.options.nrRecents && recentFonts
							? recentFonts.split(",")
							: [];
					...
					for (var f = 0; f < fonts.length; f++) {
						(tmp = fonts[f].split(":")),
							(fontType = tmp[0]),
							(fontFamily = tmp[1]),
							(font = this.allFonts[fontType][fontFamily]);
						if (!font) {
							continue;
						}
				...
				setupHtml: function () {
					...
					this.$select = jQuery('<div class="font-picker fp-select">')
						...
						.append(
							'<span class="fp-fontspec" tabindex="0">' +
								(fontSpec
									? fontSpec
				...
				show: function () {
					...
					if (!el.$select) {
						throw new Error(
							"jquery." +
								pluginName +
								": Cannot show, as I've been destroyed."
						);
					}
					el.toggleModal("show");
				},
				...
				hide: function () {
					...
					if (!el.$select) {
						throw new Error(
							"jquery." +
								pluginName +
								": Cannot hide, as I've been destroyed."
						);
					}
					el.toggleModal("hide");
				},
				...
				destroy: function () {
					...
					if (!el.$select) {
						throw new Error(
							"jquery." +
								pluginName +
								": Cannot destroy, as I've been destroyed already."
						);
					}
					el.toggleModal("hide");
					el.$select.remove();
